...
(function (root, factory) {
    ...
    if (typeof define === "function" && define.amd) {
          define('foundation/lib/tracking', ['foundation/hosts'], function (hosts) {
            factory(hosts.tagx);
          });
        ...
        if (!NYTD.env || NYTD.env === "production") {
            tagxHost = "//tagx.nytimes.com";
        } else if (NYTD.env === "staging") {
            tagxHost = "//tagx.stg.use1.nytimes.com";
        } else if (NYTD.env === "sandbox") {
            tagxHost = "//localhost:8080";
        } else {
            tagxHost = "//tagx.dev.use1.nytimes.com";
        }
        factory(tagxHost);
    }
}(this, function (host) {
    var src = host + '/?url=' +
        encodeURIComponent(location.href) + '&referrer=' +
        encodeURIComponent(document.referrer);

    // set up tagx tracking
    //TAGX.Utils.trackClicks(); -- for now lets deliver this as a tag
    // include tagx    
