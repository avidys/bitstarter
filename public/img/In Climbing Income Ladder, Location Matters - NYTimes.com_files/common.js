...
/*
 * $Id: common.js 128386 2013-06-27 23:12:51Z gopi_borra $
...
(function() {
    ...
    if (window.self != window.top && !(window.location.pathname.match(/^\/portals/) && window.location.hostname == "www.nytimes.com") && !document.referrer.match(/^https?:\/\/(?:[^?\/]+\.)?(?:localhost(?:\:[1-9][0-9]*)?|(?:nytimes|nytlabs|stumbleupon|starbucks)\.com)\//)) {
...
NYTD.loadEditionToggle = function() {
    ...
    if (toggleUS && toggleGlobal) {
        ...
        if (edition && edition[1] && edition[1].indexOf("edition|GLOBAL") != -1) {
            ...
            NYTD.updateNavigation('/index','/global/index');
        } else {
            toggleUS.innerHTML = 'U.S.'; // Deactivate U.S.
            NYTD.updateNavigation('/global','');
        }
    }
};

NYTD.updateNavigation = function( pattern ,replacement) {
    var globalSections = ["Sports", "Business", "Opinion", "Style"];
    NYTD.jQuery(document).ready( function(jQuery) {
        jQuery.each(globalSections, function(key, value) {
            var navElement = jQuery('#nav'+value);
            if (navElement.length) {
                var sectionLink = navElement.html().replace(pattern, replacement);
                navElement.html(sectionLink);
            }
        });

        var globalSubsection = jQuery('#subNav_global a');
        if (globalSubsection.length > 0) {
            var subNavGlobalUrl = globalSubsection.attr('href').replace(/(\/global)?\/index.html/, '/global/index.html');
            jQuery('#subNav_global a').attr('href', subNavGlobalUrl);
        }
    });
};
...
(function() {
    ...
    NYTD.EditionPref.setUS = function () {
        ...
        document.cookie = 'NYT-Global=;expires=Thu, 01-Jan-70 00:00:01 GMT;path=/;domain=.nytimes.com;';
    };
}());
...
(function (jQuery) {
    ...
    var getEditionToggleURL = function (isGlobalEdition) {
        ...
        if (isSectionFrontPage && hasGlobalEditionOfPage) {
            if (isGlobalEdition) {
                editionToggleURL = currentURL.replace("/global", "");
            } else {
                if (currentURL.indexOf("global") == -1) {
                    editionToggleURL = currentURL.replace(currentSection, currentSection + "/global");
                } else {
            ...
            if (isGlobalEdition) {
                ...
                editionToggleURL = 'http://global.nytimes.com';
            }
        }
                    ...
                    '<form validate id="regiModalForm" class="regiModalForm modalForm" method="post">',
                        '<div class="control email">',
                            '<div class="labelContainer">',
                                '<div class="insetV">',
                                    '<label for="regi-email">E-Mail:</label>',
                                '</div>',
                            '</div>',
                ...
                '<div class="horizontalFormControl">',
                    '<form id="loginModalForm" class="loginModalForm">',
                        '<div class="control">',
                            '<div class="labelContainer">',
                                '<div class="insetV">',
                                    '<label for="user">E-Mail/Member ID:</label>',
                                '</div>',
                            '</div>',
                            '<div class="fieldContainer">',
...
// function to display soft redirect message for iphone/ipod
(function($) {
    ...
    var noMobileCookie = (getCookie('nyt-mobile') === null);
    var showBanner     = (hasSessionStorage() && (sessionStorage.displayBanner === undefined));
	...
	if (hasSessionStorage()) {
        try {
            sessionStorage.displayBanner = true;
        }
        catch (e) {
            showBanner = false;
        }
    ...
    if ( pageHost.match('^(www|global)(.dev|.stg|)(.nytimes.com)$') ) {
        uriPattern = new RegExp('^/$|/(index.html)|^/201[2-9]/|^/(aponline|reuters)/201[2-9]/|^/(pages)/|^/(recommendations|timeswire|roomfordebate)');
    } else if ( pageHost.match('^(travel|tv|theater)(.dev|.stg|)(.nytimes.com)$') ) {
        uriPattern = new RegExp('^/201[2-9]/');
    } else if ( pageHost.match('^(6thfloor|artsbeat|atwar|bats|bits|bittman|boss|brooks|bruni|bucks|carpetbagger|cityroom|dealbook|dinersjournal|dotearth|douthat|economix|fivethirtyeight|frugaltraveler|gadgetwise|goal|india|intransit|keller|kristof|krugman|latitude|learning|newoldage|nocera|offthedribble|opinionator|pogue|publiceditor|rendezvous|runway|scientistatwork|slapshot|straightsets|takingnote|thecaucus|thechoice|thelede|therail|tmagazine|well|wheels)(.blogs)(.dev|.stg|)(.nytimes.com)$') ) {
        uriPattern = new RegExp('^/$|^/201[2-9]/');
    }
    ...
    var mobileBannerHtml = [
                            '<div id="mobileBanner" class="mobileBanner banner">',
                                '<a class="bannerClose" href="javascript:;">Close</a>',
                                '<a class="bannerMessage" href="',window.location.href,'">Switch to the mobile version of <strong>NYTimes.com</strong></a>',
                            '</div>'
                           ].join('');

    $('<link rel="stylesheet" type="text/css" href="'+NYTD.Hosts.cssHost+'/css/0.1/screen/common/modules/mobileBanner.css"/>').appendTo('head');
    $(document).ready(function () {
        //display the redirect banner for iPhone/iPod when mobile cookie is not set.
        if ( validUserAgent && hasMobileVersion && noMobileCookie && showBanner ) {
            $(mobileBannerHtml).insertBefore('#shell');
            if (hasSessionStorage()) {
                sessionStorage.displayBanner = false;
            }
        }

        $(".bannerClose").click(function() {
            $('#mobileBanner').toggleClass('hidden');
            $('#shell').css("margin-top","0px");
            setCookie('nyt-mobile', '0', {domain : 'nytimes.com', path : '/', expires : 365});
            dcsMultiTrack('DCS.dcssip', 'www.nytimes.com', 'DCS.dcsuri', '/MOW-iphone-soft-prompt-close', 'WT.ti', 'MOW iPhone soft prompt close', 'WT.z_dcsm', '1');
        });
